...
def prefix_errors(
    ...
    errors: List[Union[ErrorWrapper, dict]],
) -> List[ErrorWrapper]:
    """
    ...
    return [
        ErrorWrapper(
            error.get('exc', None) if isinstance(error, dict) else error.exc,
            loc=(*prefix, *(error.get('loc', []) if isinstance(error, dict) else error.loc)),
        )
        for error in errors
    ]
